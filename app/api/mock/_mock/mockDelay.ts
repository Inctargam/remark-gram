/**
 * Artificial latency shared by every mock API of this app (posts, subscriptions, payments).
 *
 * The in-memory stores answer instantly, so loading states — grid skeletons, the disabled
 * confirm button, `isFetchingNextPage` — flash by and cannot be checked by hand. Set
 * `MOCK_API_DELAY_MS` in `.env.local` to slow the mocks down while testing manually.
 *
 * Off by default on purpose: unit tests and story runs must not wait for it.
 */
const DELAY_ENV_VAR = 'MOCK_API_DELAY_MS'
