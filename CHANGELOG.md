...
- Добавлен виджет `src/widgets/my-payments` — вкладка «My payments» (UC-4): таблица платежей с колонками `Date of Payment`, `End date of subscription`, `Price`, `Subscription Type`, `Payment Type` и пагинацией из UI-кита. Колонка `Price` выровнена по правому краю вместе с заголовком.
- Номер страницы хранится в query-параметре `page`: страницу можно открыть по ссылке, перезагрузка её не теряет. Первая страница в url не пишется, остальные параметры вкладки (в том числе результат оплаты) сохраняются. Переход по страницам делается через `replace`, чтобы не засорять историю браузера.
- Размер страницы остаётся локальным состоянием: это настройка просмотра, а не место в списке. Смена размера возвращает на первую страницу.
- Состояния таблицы: скелетон при первой загрузке, строка-заглушка при отсутствии платежей, `Alert` при ошибке запроса. При смене страницы прежние строки остаются на экране, пока грузятся новые.
- Список вариантов «сколько показывать» ограничен `[10, 20, 30, 50]`: мок-API отклоняет `pageSize` больше 50. На макете в селекте `100` — вернуть вместе с настоящим бэкендом.
- Во временном роуте настроек вкладка «My payments» разблокирована и подключена, виджет обёрнут в `Suspense` — он читает номер страницы из query-строки.

#### Entities

- `SUBSCRIPTION_PERIOD_LABELS` в `entities/subscription` — период сам по себе (`1 day`, `7 days`, `1 month`), отдельно от подписи плана с ценой.
- `PAYMENT_PROVIDER_LABELS` в `entities/payment` — названия провайдеров в фирменном регистре (`Stripe`, `PayPal`).

#### Shared UI

- Заполнен публичный экспорт `src/shared/ui/pagination/index.ts`: файл был пустым, и импорт `@/shared/ui/pagination` не компилировался.

#### Tests

- Юнит-тесты разбора и сборки query-параметра страницы: некорректные значения падают на первую страницу, `page=1` из url убирается, чужие параметры сохраняются.
- Сторис вкладки: одна страница, несколько страниц с кликом по номеру, пусто, загрузка, ошибка.

#### Verification

- `pnpm test:unit` прошёл: 42 файла, 235 тестов.
- `pnpm exec vitest run --project storybook` прошёл: 52 файла, 232 теста.
- `pnpm exec tsc --noEmit` и `pnpm build` прошли; `eslint` по изменённым файлам — 0 ошибок.
- Мок-API проверен на запущенном dev-сервере: `GET /api/mock/payments?page=2&pageSize=10` отдаёт `totalPages: 2`, `totalCount: 14` и 4 строки.
- Вкладку не удалось посмотреть в браузере (расширение не сняло скриншот дев-сервера); SSR-разметка проверена запросом, клиентский рендер — сторями.

### 2026-08-11 (2)

#### Subscriptions

- Добавлена фича `src/features/cancel-auto-renewal` — чекбокс `Auto-Renewal` под блоком текущей подписки (UC-2). Состояние меняется оптимистично и откатывается при ошибке запроса, под чекбоксом появляется сообщение об ошибке.
- Оптимистичное обновление пишет в кэш весь статус аккаунта: флаг у последней подписки очереди и `nextPaymentAt` (дата хвоста при включённом автопродлении, пусто при выключенном). Иначе чекбокс двигался бы сразу, а дата следующего платежа — с задержкой на запрос.
- Чекбокс презентационный: состояние берётся из кэша подписки, поэтому виджет и фича не расходятся.

#### Account Management

- Блок `Current Subscription` показывает очередь подписок: строка на каждую, подписи колонок один раз сверху, `Next payment` заполнен только у последней — остальные уже оплачены (UC-3).
- Из-за этого блок переехал с карточки на `Table` из UI-кита: с несколькими строками подписи пришлось бы повторять. Вид карточки сохранён (шапка без заливки и без разделителя, ширина по содержимому).
- Чекбокс автопродления передаётся во вью слотом `autoRenewalSlot`, поэтому вью остаётся без запросов к API.
- Покупка при активной подписке (UC-3) отдельного кода не потребовала: платёжный флоу не зависит от наличия подписки, очередь и снятие автопродления у предыдущей делает мок-стор.

#### Tests

- Юнит-тесты `applyAutoRenewal`: выключение и включение флага у хвоста очереди, неприкосновенность остальных подписок, аккаунт без подписок.
- `selectCurrentSubscription` переименован в `selectSubscriptionQueue`, тесты переписаны под очередь строк.
- Сторис чекбокса автопродления: выключение, включение, откат с сообщением при ошибке сервера. Добавлена стори вкладки с очередью из двух подписок.

#### Verification

- `pnpm test:unit` прошёл: 41 файл, 224 теста.
- `pnpm exec tsc --noEmit`, `pnpm build` прошли; `eslint` и `stylelint` по изменённым слайсам — 0 ошибок.
- `pnpm exec vitest run --project storybook` по фиче и виджету — 11 тестов. Полный `pnpm test:storybook` на этом этапе не гонялся.
- Мок-API проверен запросами к запущенному dev-серверу: две покупки подряд дают очередь с автопродлением только у последней подписки, `PATCH /auto-renewal` снимает флаг и обнуляет `nextPaymentAt`.

### 2026-08-11

#### Payments

- Добавлена фича `src/features/buy-subscription` — платёжный флоу вкладки «Account Management» целиком: согласие, создание сессии, уход на платёжный сервис и разбор результата после возврата.
- Модалка согласия `Create payment` собрана на `Modal`, а не на `ConfirmDialog`: по макету у неё одна кнопка `OK` и чекбокс `I agree` в одной строке с ней. `OK` заблокирован, пока не проставлен чекбокс; закрытие крестиком или по фону — отмена без платежа.
- Результат оплаты передаётся query-параметром `?payment=success|failed`, а не состоянием: возврат от платёжного сервиса — полная перезагрузка страницы. После закрытия модалки параметр снимается из url, поэтому перезагрузка не показывает результат заново.
- Модалки результата: `Success` / `Payment was successful!` / `OK` и `Error` / `Transaction failed. Please, write to support` / `Back to payment`. Через `Error` показывается и несозданная сессия: для пользователя это тот же отказ.
- Переход на оплату сделан полной навигацией (`window.location.assign`), а не роутером: настоящий провайдер держит страницу на своём домене, и при переезде на реальный Stripe переход с возвратом менять не придётся.
- Провайдер остаётся параметром флоу: обе кнопки (`Stripe` и слот `PayPal`) зовут один и тот же сценарий с разным `PaymentProvider`.
- После успешной оплаты инвалидируются `subscription` и `payments`.

#### Mock Checkout

- Добавлены слайс `src/pages/mock-checkout` и роут `app/payments/mock-checkout/page.tsx` — заглушка «внешнего» платёжного сервиса с кнопками `Pay` и `Cancel`. Она отчитывается о результате в мок-API и возвращает браузер на `returnUrl`. Роут лежит вне группы `(main)`: по сценарию пользователь уже вне приложения.
- `returnUrl` приходит в query-строке, поэтому принимается только same-origin — иначе страница работала бы как open redirect. Всё остальное уводит на страницу настроек.
- Неизвестная или уже завершённая сессия возвращает пользователя с `payment=failed`, а не показывает ошибку на самой заглушке.
- **Временно:** заглушка и её роут снимаются вместе с мок-API платежей.

#### Account Management

- Кнопки платёжных сервисов на вкладке заработали: виджет передаёт обработчик выбора провайдера и рендерит модалки согласия и результата.
- Виджет во временном роуте настроек обёрнут в `Suspense` — он читает результат оплаты из query-строки.

#### Tests

- Юнит-тесты: разбор query-параметра результата и сборка адреса возврата (сохранение чужих параметров, замена прежнего результата, отказ от чужого домена и от нечитаемого url).
- Сторис модалки согласия (`OK` заблокирован без чекбокса и во время запроса, разблокирован после, отмена крестиком) и модалок результата (успех, ошибка, отсутствие результата).

#### Verification

- `pnpm exec tsc --noEmit`, `pnpm build` прошли.
- `pnpm exec eslint` и `stylelint` по новым слайсам — 0 ошибок.
- `pnpm test:unit` прошёл: 40 файлов, 220 тестов. `pnpm test:storybook` прошёл: 50 файлов, 223 теста.
- Сквозной прогон в `pnpm dev` пройден вручную: покупка с успехом и с отказом.

### 2026-08-10

#### Payments

- Добавлена сущность `src/entities/subscription`: типы аккаунта и подписки, каталог планов (`day` / `week` / `month`, цены-заглушки в центах в одном массиве `SUBSCRIPTION_PLANS`), query keys и `useCurrentSubscriptionQuery`.
- Добавлена сущность `src/entities/payment`: тип строки истории платежей, постраничная `usePaymentsQuery` с `keepPreviousData`, чтобы таблица не схлопывалась при переключении страниц.
- Весь обмен с API подписок и платежей идёт через `entities/*/api/*Api.ts` — единственные файлы, которые переедут на реальный бэкенд. Пути моков повторяют будущие реальные и отличаются префиксом: `/api/mock/subscriptions` против `/api/v1/subscriptions`. Переключатель — `NEXT_PUBLIC_PAYMENTS_API_MOCK` (`true` в `.env.local`), читается в момент вызова.
- Эндпоинтов подписок и платежей на бэкенде нет, поэтому весь флоу оплаты — мок: `src/shared/api/mock/subscriptionsStore.ts` по образцу `postsStore.ts` (состояние на `globalThis`, переживает hot reload, сид детерминированный).
- Правила домена держатся в сторе, а не в UI: новая подписка встаёт в конец очереди (стартует, когда истекает текущая), автопродление остаётся включённым только у последней подписки, `nextPaymentAt` считается от хвоста очереди и обнуляется при выключенном автопродлении.
- Повторное завершение платёжной сессии отбивается `409`: перезагрузка страницы возврата не может создать вторую подписку.
- Мок-роуты: `GET /api/mock/subscriptions/current`, `POST /api/mock/subscriptions/checkout`, `POST /api/mock/subscriptions/checkout/{sessionId}/complete`, `PATCH /api/mock/subscriptions/auto-renewal`, `GET /api/mock/payments?page=&pageSize=`.
- Сид: аккаунт `personal` без активных подписок и 14 строк истории платежей — стартовое состояние сценария покупки и данные для пагинации.

#### Tooling

- `withMockDelay` переехал из `app/api/mock/posts/_mock/` в общий `app/api/mock/_mock/`, потому что задержка нужна всем мок-API. Переменная окружения переименована: `POSTS_API_MOCK_DELAY_MS` → `MOCK_API_DELAY_MS`. Кто задавал её локально — нужно обновить `.env.local`.
- В `vitest.config.ts` в `coverage.include` добавлены слайсы подписок и платежей, иначе порог 80% измерял бы только код постов.

#### Account Management

- Добавлен виджет `src/widgets/account-management` — вкладка настроек «Account Management»: тип аккаунта, список планов, блок текущей подписки и кнопки платёжных сервисов. Пропов не принимает: каркасу страницы настроек достаточно отрендерить его в своей панели.
- Порядок блоков и заголовки взяты с макета и зависят от состояния: без подписки `Account type:` → `Your subscription costs:`, с активной подпиской `Current Subscription:` → `Account type:` → `Change your subscription:`.
- Блок текущей подписки — карточка с колонками `Expire at` / `Next payment`, а не таблица. Дата истечения берётся с хвоста очереди подписок: после покупки второй подписки поверх активной доступ кончается в конце очереди. При выключенном автопродлении в `Next payment` стоит прочерк.
- Переключение на `Business` — локальный выбор до оплаты: сервер узнаёт о нём только после успешного платежа. Пункт `Personal` заблокирован, пока подписка активна, — понижение типа аккаунта делает бэкенд.
- Кнопки `PayPal` — `Or` — `Stripe` отрисованы, но заблокированы: обработчик выбора провайдера появится вместе с платёжным флоу. Обе кнопки зовут один обработчик с разным `PaymentProvider`, так что вторая ветка оплаты не форкает флоу.
- `src/shared/lib/date/formatShortDate.ts` — формат дат `12.02.2022` (UTC), общий для вкладки подписок и будущей таблицы платежей.
- **Временно:** роут `app/(main)/profile/settings/page.tsx` и запись `ROUTES.profileSettings`. Страница настроек с табами делается в другой задаче; оба места помечены `TEMPORARY` и снимаются при интеграции.

#### Shared UI

- Добавлен компонент `src/shared/ui/table` — компаунд `Table.Root / Head / Body / Row / HeadCell / Cell` в стиле существующего `Tabs`. Нужен блоку «Current subscription» и вкладке «My payments», в UI-ките таблицы не было.
- Семантика разведена: `HeadCell` рендерит `<th scope="col">`, `Cell` — `<td>`, поэтому шапка читается скринридером как шапка.
- `Table.Root` оборачивает `<table>` в контейнер с горизонтальным скроллом: на узких экранах прокручивается таблица, а не страница. Класс контейнера задаётся отдельным пропом `wrapperClassName`.
- Состояния данных встроены в компонент, чтобы не дублировать их в виджетах: `Table.Empty` (строка на всю ширину, обязательный `colSpan`) и `Table.Skeleton` (`columns`, `rows` по умолчанию 3). Скелетон скрыт от скринридеров, состояние загрузки объявляется через `aria-busy` на `Table.Root`; пульсация отключается при `prefers-reduced-motion`.
- Цвета, типографика и отступы — только из токенов проекта.

#### Tests

- Тесты стора: сид, очередь подписок, инвариант единственного автопродления, отказ оплаты, повторное завершение сессии, пагинация платежей.
- Тесты мок-хендлеров: валидация плана, провайдера, `returnUrl`, `outcome` и флага автопродления; коды `400` / `404` / `409`.
- Тесты слоя запросов обеих сущностей: пути мока и реального бэкенда, методы и тела запросов.
- Тесты `formatShortDate` (формат, UTC, нераспознанная дата) и `selectCurrentSubscription` (пустая очередь, хвост очереди, выключенное автопродление).
- Сторис вкладки Account Management: Personal, Business без подписки, Business с активной подпиской, подписка без автопродления, загрузка, ошибка, состояние без платёжного флоу.

#### Verification

- `pnpm test:unit` прошёл: 36 файлов, 205 тестов.
- `pnpm lint` — 0 ошибок; остаются существующие предупреждения проекта в `src/shared/api/openapi/schema.d.ts` и `src/features/sign-up/model/useSignUpForm.ts`.
- `pnpm exec tsc --noEmit` прошёл без ошибок.
- `pnpm build` прошёл, все пять мок-роутов зарегистрированы.
- `pnpm test:coverage` — 98.24% строк, 94.26% ветвей.
- `pnpm test:storybook` прошёл: 48 файлов, 216 тестов (включая стори `Table` и вкладки Account Management).

#### Notes

- Компонента таблицы в дизайн-системе Figma нет (`search_design_system` вернул пустой список), поэтому отступы и рамки `Table` взяты по аналогии с соседями по UI-киту.
- Вкладка Account Management сверена с макетами по скриншотам; вживую в браузере не прогонялась — сквозной прогон будет вместе с платёжным флоу.
- Цены планов, заложенные до макета, совпали с ним дословно — `SUBSCRIPTION_PLANS` менять не пришлось.
- Оплата целиком мок, Stripe SDK и ключи не подключаются: страница-заглушка платёжного сервиса `/payments/mock-checkout` появится следующим этапом, до неё сквозной прогон оплаты руками невозможен.
- Цены и состав планов — заглушки, сверяются с макетом на этапе вёрстки.
- `entities/payment` импортирует `PaymentProvider` и `SubscriptionPeriod` из `entities/subscription` через публичный `index.ts` — осознанный кросс-импорт внутри слоя вместо дублирования union-типов.
