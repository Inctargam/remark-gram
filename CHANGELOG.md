...
- `pnpm test:storybook` прошёл: 47 файлов, 209 тестов (включая пять сторей нового `Table`).

#### Notes

- Компонента таблицы в дизайн-системе Figma нет (`search_design_system` вернул пустой список), поэтому отступы и рамки `Table` взяты по аналогии с соседями по UI-киту и подлежат сверке с макетом на этапе вёрстки вкладок.
