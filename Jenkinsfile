...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 4324
        NAMESPACE = "remark-gram-com"
        REGISTRY_HOSTNAME = "ykvachonak"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "remark-gram-front"
        DEPLOYMENT_NAME = "remark-gram-front-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'remark-gram-com') {
